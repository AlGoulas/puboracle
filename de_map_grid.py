...
# Create shapely Point object from lon lat
from shapely.geometry import Point
import geoplot
c = [Point(coord[0], coord[1]) for coord in lon_lat]
c_dict = {'geometry':c}
lon_lat_df = pd.DataFrame (c_dict, columns=['geometry'])
# Create attribute total bounds as an array with minx min y maxx maxy 
# minx = min(lon_lat, key=lambda a:a[0])[0]
# miny = min(lon_lat, key=lambda a:a[1])[1]
# maxx = max(lon_lat, key=lambda a:a[0])[0]
# maxy = max(lon_lat, key=lambda a:a[1])[1]
# lon_lat_df.total_bounds = np.asarray([minx, miny, maxx, maxy])
lon_lat_df.total_bounds = plz_shape_df.total_bounds
ax = geoplot.kdeplot(
    lon_lat_df, 
    cmap = cmap, 
...
gplt.polyplot(plz_shape_df, ax = ax)
